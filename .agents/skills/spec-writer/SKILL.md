...
1. **Context Gathering & Pre-flight Rules**
   ...
   - **MANDATORY**: You MUST read `rules/package-architecture.md` to learn the strict package layer rules and import matrices before you design any technical architecture.
...
2. **Drafting the Epic**
   ...
   - Ensure the technical architecture strictly adheres to the project's dependency and import rules.
...
3. **Generating ADRs**
   ...
   - **Package Boundary Pre-flight Rule**: Before specifying technical architecture, you MUST read `rules/package-architecture.md` and verify layer placement (e.g., external SDKs/clients -> `packages/infrastructure/`, UI -> `apps/web/`, pure types -> `packages/shared/`).
...
4. **Defining Implementation Steps & Definitions of Done (DoD)**
   ...
   - Good DoDs for AI include executable tasks: "Start the web app using `npm run dev`, navigate to `/feature-url`, click the 'Submit' button, and verify the resulting success modal appears using screenshots."
