...
1. Mandatory Pre-reads: `CONTRIBUTING.md`, `rules/coding-style.md`, `rules/testing.md`, relevant `GEMINI.md`, and the implementation plan. If touching `apps/web/`, also read `rules/styling.md`. Respect the architecture (`specs/system_architecture.md`).
2. TDD Cycle: 
...
5. End State: Run `make check`. If all checks pass, open a PR using `gh pr create` and link the issue.
6. Update `.agents/memory/milestones-todos.md` on completion.
