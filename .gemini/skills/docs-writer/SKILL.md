...
- **Callouts**: Use GitHub-flavored markdown alerts to highlight important
  information. To ensure the formatting is preserved by `make format`, place
...
3.  **Link check:** Verify all new and existing links leading to or from
    ...
    it are updated.
4.  **Format:** If `make format` fails, it may be necessary to run `make setup` first to ensure all formatting dependencies are available. Once all
    changes are complete, ask to execute `make format` to ensure consistent
