...
5. **Single Project Context Seam:** All project-scoped route handlers and Server Actions MUST call `requireProjectContext({ id: projectId })` from `@/lib/auth/project-context`. Never write ad-hoc project ownership validation or session checks in route controllers.
6. **Deep Lifecycle Seam:** To delete a project or material, presentation controllers MUST invoke `deleteProjectLifecycle` or `deleteMaterialLifecycle` from `@/lib/materials/lifecycle`. Directly executing low-level delete queries (e.g. `deleteProjectById`, `deleteMaterialById`) from the presentation layer is an architectural violation detected by `pnpm scan:arch`.
7. **Typed Domain Errors & Actionable UI Error Boundaries:** Standardize on `AppError` (`ChatbotError`) codes (`${ErrorType}:${Surface}`). API controllers return uniform JSON responses (`{ type, surface, statusCode, message }`). UI routes must provide actionable error boundaries (`app/error.tsx`, `app/global-error.tsx`, `app/projects/[projectId]/error.tsx`) featuring clear recovery CTAs and fallback navigations.
